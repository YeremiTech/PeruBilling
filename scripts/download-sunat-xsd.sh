...
# Los paquetes oficiales han cambiado de estructura/nombre con el tiempo; el runtime
# localiza recursivamente estos entrypoints, por eso validamos por basename.
...
# Hashes relativos a la raíz XSD para que el manifest sea portable entre hosts.
(
  cd "$STAGE"
...
# Sustitución atómica a nivel de directorios: la instalación activa no se toca
# hasta que descargas, checksums, ZIPs y entrypoints hayan sido validados.
mkdir -p "$ROOT"
BACKUP="$TMP/previous"
mkdir -p "$BACKUP"
