...
1. Identidad del emisor con el isotipo oficial de PeruBilling y el nombre comercial o razón social.
...
8. Condición de pago y cuotas cuando correspondan.
...
9. QR SUNAT, DigestValue y acceso público.
10. Cierre compacto de representación impresa.
...
- El isotipo proviene del SVG canónico incluido en `src/main/resources/branding/perubilling-mark.svg` y conserva sus tres colores de marca.
...
- El cálculo de altura de `THERMAL_80` considera cantidad de ítems, cuotas, referencias y longitud aproximada de textos; si el renderer genera una segunda hoja por redondeo, el alto se incrementa y se vuelve a renderizar hasta obtener una página continua.
...
## v6: integridad del render e identificación del artefacto

La revisión v6 reincorpora el A4 a sangre de v4 y corrige la cabecera y la altura final del ticket. Tras una estimación conservadora, PDFBox mide el final del texto de la hoja térmica y el motor vuelve a renderizar con margen adicional para el corte. Los PDF nuevos registran en la metadata la versión `PRINT-V6-20260922` y el formato `A4`/`THERMAL_80`. Consulta [`PRINTING_V6.md`](PRINTING_V6.md) para pruebas y precauciones sobre artefactos previamente guardados.
