...
API modular de facturación electrónica para Perú orientada a integración con POS, ERP, ecommerce y sistemas de ventas. Versión de trabajo: `0.8.1-SNAPSHOT`.
...
| Factura A4 | Ticket térmico 80 mm |
|---|---|
| ![Factura A4](docs/assets/portada-a4.png) | ![Ticket 80 mm](docs/assets/portada-ticket.png) |

PDF de referencia:

- [Factura A4 de ejemplo](docs/examples/factura-a4-ejemplo.pdf)
- [Boleta térmica 80 mm de ejemplo](docs/examples/boleta-thermal-80-ejemplo.pdf)

> Documentos generados en ambiente `LOCAL` con datos sintéticos solo para referencia visual.
...
Si un envío queda en `SUBMISSION_UNKNOWN`, el sistema no lo reenvía automáticamente. Tras verificar operativamente que SUNAT no aceptó el envío anterior, un operador puede autorizar un reintento controlado:

```http
POST /api/v1/daily-summaries/{id}/retry-unknown?confirmRetry=true
```
...
- si la baja es rechazada o falla definitivamente, se recupera el último estado fiscal estable (`ACCEPTED` u `OBSERVED`) en lugar de convertir siempre el comprobante a `ACCEPTED`;
- un envío `SUBMISSION_UNKNOWN` solo puede reencolarse con confirmación explícita después de la verificación operativa correspondiente.

```http
POST /api/v1/documents/{id}/void/retry-unknown?confirmRetry=true
```
...
```bash
SPRING_PROFILES_ACTIVE=prod java -jar target/perubilling-0.8.1-SNAPSHOT.jar
```
...
La versión `0.8.1-SNAPSHOT` incorpora el hardening técnico vigente para preproducción:
...
Para operación y despliegue use `OPERATIONS.md`. Para el procedimiento de evidencia BETA use `docs/SUNAT_BETA_CERTIFICATION.md`.


## Representaciones PDF

**Impresión v6 consolidada:** véase [`docs/PRINTING_V6.md`](docs/PRINTING_V6.md) para las medidas de A4 y 80 mm, el diagnóstico de la regresión v5, los iconos alineados por fila, la altura recortada del ticket y las pruebas después del despliegue.
...
El isotipo canónico se conserva en `src/main/resources/branding/perubilling-mark.svg`. La representación PDF utiliza la misma geometría y la paleta `#053E58`, `#0B5675` y `#1AB09B`, embebida a alta resolución para mantener nitidez en A4 y `THERMAL_80` sin depender de recursos externos.

El ticket térmico se renderiza como una única hoja continua de 80 mm. En v6, además de crecer cuando hay desborde, el generador mide el final del contenido con PDFBox y vuelve a renderizar con una altura ajustada y margen de corte. Los PDF nuevos incluyen metadatos `PRINT-V6-20260922` y el layout para detectar descargas de artefactos antiguos. Los PDF ya almacenados no cambian al desplegar una plantilla: prueba con un comprobante NUEVO en LOCAL.

### Robustez de emisión M2M y baja

La persistencia de `Idempotency-Key` fuerza primero la materialización del comprobante y registra después la clave idempotente mediante SQL transaccional explícito. La Comunicación de Baja bloquea el documento con `PESSIMISTIC_WRITE` y registra el lote con inserción idempotente por documento, de modo que un reintento seguro devuelve el lote existente en vez de producir un `409` artificial.
