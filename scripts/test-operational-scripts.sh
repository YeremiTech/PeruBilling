...
env "${COMMON_ENV[@]}" BACKUP_DIR="$BACKUPS" "$ROOT_DIR/scripts/backup.sh" >/dev/null
...
env "${COMMON_ENV[@]}" BACKUP_PATH="$backup_path" RESTORE_CONFIRM=YES \
  "$ROOT_DIR/scripts/restore.sh" >/dev/null

env "${COMMON_ENV[@]}" "$ROOT_DIR/scripts/verify-artifacts.sh" >/dev/null
